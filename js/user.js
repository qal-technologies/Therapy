...
import { getUserData, getUserPayments } from './database.js';
...
function renderUserProfile(user, payments) {
    if (!user) return;
...
window.addEventListener('DOMContentLoaded', () => {
    handleAuthStateChange(async (user) => {
        ...
        body.style.display = 'block'; // Show body after auth check
        ...
        if (user) {
            ...
            try {
                ...
                renderUserProfile(userData, userPayments);
                setupEventListeners();
            } catch (error) {
            ...
            setTimeout(() => {
                window.location.replace("/html/regs/Signup.html"); // Use replace to prevent back button
            }, 1500);
        }
    });
});